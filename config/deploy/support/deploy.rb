...
namespace :deploy do
  ...
  task :immortalize_setup, :roles => :app do
    run "#{$IMMORTALIZE} setup"
    run "#{$IMMORTALIZE} stop all"
    run "#{$IMMORTALIZE} remove all"
  end
  ...
  desc "Reload Merb & services"
  task :reload => [:immortalize_setup, :start]
end
